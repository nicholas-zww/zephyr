...
#define WHD_THREAD_POLL_TIMEOUT      (100)
...
whd_result_t whd_bus_sdio_wait_for_wlan_event(whd_driver_t whd_driver, cy_semaphore_t *transceive_semaphore)
{
    ...
    else
    {
        ...
        if ((result == CY_RTOS_TIMEOUT) && (timeout_ms > WHD_THREAD_POLL_TIMEOUT))
        {
            /*
             * AP6212 on STM32H7 can miss SDIO function interrupts during early
             * post-download IOCTL traffic. Prod the WLAN once, then force the
             * WHD thread to poll the mailbox when this wait returns.
             */
            whd_result_t poke_result = whd_bus_poke_wlan(whd_driver);

            whd_driver->thread_info.bus_interrupt = WHD_TRUE;

            if (poke_result == WHD_SUCCESS)
            {
                result = cy_rtos_get_semaphore(transceive_semaphore,
                                               (uint32_t)MIN_OF(timeout_ms,
                                                                WHD_THREAD_POKE_TIMEOUT),
                                               WHD_FALSE);
            }
        }
    }
