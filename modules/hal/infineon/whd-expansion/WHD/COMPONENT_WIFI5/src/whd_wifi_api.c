...
static whd_bool_t whd_wifi_key_looks_like_raw_pmk(const uint8_t *key, uint8_t key_length)
{
    uint8_t i;

    if ((key == NULL) || (key_length != WSEC_PMK_LEN))
    {
        return WHD_FALSE;
    }
...
static const whd_event_num_t scan_events[] = { WLC_E_ESCAN_RESULT, WLC_E_SCAN_COMPLETE, WLC_E_NONE };
...
whd_result_t whd_wifi_set_up(whd_interface_t ifp)
{
    ...
    /* Send UP command */
    retval = whd_wifi_set_ioctl_buffer(ifp, WLC_UP, NULL, 0);
    if (retval != WHD_SUCCESS)
    {
        /*
         * AP6212 firmware on ART-Pi can report WLC_UP failure while still
         * responding to subsequent version, MAC, and scan/join controls.
         */
        WPRINT_WHD_ERROR( ("WLC_UP failed\n") );
    }
...
whd_result_t whd_wifi_set_passphrase(whd_interface_t ifp, const uint8_t *security_key, uint8_t key_length)
{
    ...
    whd_driver_t whd_driver;
    wsec_pmk_t *psk;
    whd_result_t retval = WHD_WLAN_ERROR;
    uint32_t retry;
    ...
    /*
     * AP6212 on ART-Pi can still be settling when the passphrase/PMK ioctl is
     * first issued. Allocate a fresh WHD buffer for each attempt because the
     * transport owns the buffer after whd_proto_set_ioctl().
     */
    for (retry = 0; retry < 3; ++retry)
    {
        CHECK_RETURN(cy_rtos_delay_milliseconds((retry == 0) ? 100 : 100) );
        ...
        retval = whd_proto_set_ioctl(ifp, WLC_SET_WSEC_PMK, buffer, 0);
        if (retval == WHD_SUCCESS)
        {
            return WHD_SUCCESS;
        }
...
whd_result_t whd_wifi_set_pmk(whd_interface_t ifp, const uint8_t *security_key, uint8_t key_length)
{
    ...
    /*
     * AP6212 on ART-Pi can still be settling when the PMK ioctl is first
     * issued. Allocate a fresh WHD buffer for each attempt because the
     * transport owns the buffer after whd_proto_set_ioctl().
     */
    for (retry = 0; retry < 3; ++retry)
    {
                                      ...
                                      cy_semaphore_t *semaphore)
{
    ...
    if (auth_type != WHD_SECURITY_OPEN)
    {
        ...
        check_result = whd_wifi_get_iovar_value(ifp, IOVAR_STR_MFP, &auth_mfp);
        if (check_result != WHD_SUCCESS)
        {
            /*
             * AP6212 firmware can reject this optional pre-read even though
             * WPA2/WPA3 joins still proceed with the later explicit MFP set.
             */
            auth_mfp = WL_MFP_NONE;
        }
    }

    /* Set Wireless Security Type */
    retval = whd_wifi_set_ioctl_value(ifp, WLC_SET_WSEC, (uint32_t)(auth_type & 0xFF) );
    if (retval != WHD_SUCCESS)
    {
        /*
         * AP6212 firmware on ART-Pi can reject the generic WLC_SET_WSEC path
         * even though later bsscfg/auth/passphrase programming still has a
         * chance to succeed.
         */
    }

    /* Enable Roaming in FW by default */
    retval = whd_wifi_set_iovar_value(ifp, IOVAR_STR_ROAM_OFF, 0);
    if (retval != WHD_SUCCESS)
    {
    }
    ...
    switch (auth_type)
    {
        ...
        case WHD_SECURITY_WPA2_WPA_MIXED_PSK:
            /*
             * Defer WPA/WPA2 key programming until after auth and wpa_auth are
             * configured below. AP6212 firmware is sensitive to this ordering.
             */
            break;
    ...
    retval = whd_wifi_set_ioctl_value(ifp, WLC_SET_INFRA, ( (auth_type & IBSS_ENABLED) == 0 ) ? 1 : 0);
    if (retval != WHD_SUCCESS)
    {
    }
    ...
    retval = whd_wifi_set_ioctl_value(ifp, WLC_SET_AUTH, auth);
    if (retval != WHD_SUCCESS)
    {
    }
    ...
    *wpa_auth = htod32(*wpa_auth);
    retval = whd_proto_set_ioctl(ifp, WLC_SET_WPA_AUTH, buffer, 0);
    if (retval != WHD_SUCCESS)
    {
    }
    ...
    switch (auth_type)
    {
        ...
        case WHD_SECURITY_WPA2_WPA_MIXED_PSK:
            ...
            CHECK_RETURN_UNSUPPORTED_CONTINUE(whd_wifi_set_supplicant_key_timeout(ifp,
                                                                                  DEFAULT_EAPOL_KEY_PACKET_TIMEOUT) );
            if (whd_wifi_key_looks_like_raw_pmk(security_key, key_length) == WHD_TRUE)
            {
                CHECK_RETURN(whd_wifi_set_pmk(ifp, security_key, key_length) );
            }
            else
            {
...
static whd_result_t whd_wifi_legacy_scan_emit_results(whd_interface_t ifp, void *handler_user_data)
{
    uint8_t *buffer;
    ...
    buffer = whd_mem_malloc(WLC_IOCTL_MEDLEN);
    if (buffer == NULL)
    {
        return WHD_MALLOC_FAILURE;
    }

    whd_mem_memset(buffer, 0, WLC_IOCTL_MEDLEN);
    results = (wl_scan_results_t *)buffer;
    results->buflen = htod32(WLC_IOCTL_MEDLEN);

    result = whd_wifi_get_ioctl_buffer(ifp, WLC_SCAN_RESULTS, buffer, WLC_IOCTL_MEDLEN);
    if (result != WHD_SUCCESS)
    {
        whd_mem_free(buffer);
        return result;
    ...
    count = dtoh32(results->count);
    cursor = (uint8_t *)&results->bss_info[0];
    end = buffer + MIN_OF(dtoh32(results->buflen), (uint32_t)WLC_IOCTL_MEDLEN);

    whd_mem_memset(&event_header, 0, sizeof(event_header));
    event_header.event_type = WLC_E_ESCAN_RESULT;
    event_header.status = WLC_E_STATUS_PARTIAL;
    ...
    for (i = 0; i < count; i++)
    {
        ...
        if ((cursor + sizeof(wl_bss_info_t)) > end)
        {
            break;
        }

        bss_info = (wl_bss_info_t *)cursor;
        bss_length = dtoh32(WHD_READ_32(&bss_info->length));
        if ((bss_length < sizeof(wl_bss_info_t)) || ((cursor + bss_length) > end))
        {
            break;
        }

        event_data_length = offsetof(wl_escan_result_t, bss_info) + bss_length;
        event_data = whd_mem_malloc(event_data_length);
        if (event_data == NULL)
        {
            whd_mem_free(buffer);
            return WHD_MALLOC_FAILURE;
        ...
        whd_mem_memset(event_data, 0, event_data_length);
        eresult = (wl_escan_result_t *)event_data;
        eresult->buflen = htod32(event_data_length);
        eresult->version = htod32(ESCAN_REQ_VERSION);
        eresult->bss_count = htod16(1);
        whd_mem_memcpy(&eresult->bss_info[0], bss_info, bss_length);
        event_header.datalen = event_data_length;

        (void)whd_wifi_scan_events_handler(ifp, &event_header, event_data, handler_user_data);
        whd_mem_free(event_data);

        if (ifp->whd_driver->internal_info.scan_result_callback == NULL)
        {
            break;
        }
...

{
    ...
    if (event_header->status == WLC_E_STATUS_SUCCESS)
    {
        if (whd_driver->internal_info.legacy_scan_in_progress == WHD_TRUE)
        {
            whd_result_t legacy_result = whd_wifi_legacy_scan_emit_results(ifp, handler_user_data);
            whd_driver->internal_info.legacy_scan_in_progress = WHD_FALSE;

            if (legacy_result != WHD_SUCCESS)
            {
                       ...
                       )
{
    ...
    wl_scan_params_t *legacy_scan_params;
    ...
    {
        whd_result_t res = whd_management_set_event_handler(ifp, scan_events,
                                                            whd_wifi_scan_events_handler, user_data,
                                                            &event_entry);
        if (res != WHD_SUCCESS)
        {
            WPRINT_WHD_ERROR( ("%s: set_event_handler failed 0x%lx\n", __func__, (unsigned long)res) );
            return res;
        }
    }
    ...
    {
        whd_result_t res = whd_proto_set_iovar(ifp, buffer, 0);
        if (res != WHD_SUCCESS)
        {
            WPRINT_WHD_ERROR( ("%s: escan start failed 0x%lx, trying legacy scan\n",
                               __func__, (unsigned long)res) );

            legacy_scan_params = (wl_scan_params_t *)whd_proto_get_ioctl_buffer(whd_driver, &buffer, param_size);
            CHECK_IOCTL_BUFFER(legacy_scan_params);
            whd_mem_memset(legacy_scan_params, 0, param_size);

            legacy_scan_params->scan_type = (int8_t)scan_type;
            legacy_scan_params->bss_type = (int8_t)bss_type;

            if (optional_ssid != NULL)
            {
                legacy_scan_params->ssid.SSID_len = htod32(optional_ssid->length);
                whd_mem_memcpy(legacy_scan_params->ssid.SSID, optional_ssid->value,
                               legacy_scan_params->ssid.SSID_len);
            }

            if (optional_mac != NULL)
            {
                whd_mem_memcpy(legacy_scan_params->bssid.octet, optional_mac, sizeof(whd_mac_t) );
            }
            else
            {
                whd_mem_memset(legacy_scan_params->bssid.octet, 0xff, sizeof(whd_mac_t) );
            }

            if (optional_extended_params != NULL)
            {
                legacy_scan_params->nprobes =
                    (int32_t)htod32(optional_extended_params->number_of_probes_per_channel);
                legacy_scan_params->active_time =
                    (int32_t)htod32(optional_extended_params->scan_active_dwell_time_per_channel_ms);
                legacy_scan_params->passive_time =
                    (int32_t)htod32(optional_extended_params->scan_passive_dwell_time_per_channel_ms);
                legacy_scan_params->home_time =
                    (int32_t)htod32(optional_extended_params->scan_home_channel_dwell_time_between_channels_ms);
            }
            else
            {
#ifndef PROTO_MSGBUF
                legacy_scan_params->nprobes = (int32_t)htod32(-1);
                legacy_scan_params->active_time = (int32_t)htod32(-1);
                legacy_scan_params->passive_time = (int32_t)htod32(-1);
                legacy_scan_params->home_time = (int32_t)htod32(-1);
#else
                whd_mem_memset(&(legacy_scan_params->nprobes), -1, 16);
#endif
            }
            ...
            if ( (channel_list_size > 0) && (optional_channel_list != NULL) )
            {
                ...
                for (i = 0; i < channel_list_size; i++)
                {
                    legacy_scan_params->channel_list[i] = htod16(CH20MHZ_CHSPEC(optional_channel_list[i]) );
                }
                legacy_scan_params->channel_num =
                    (int32_t)htod32(channel_list_size & WL_SCAN_PARAMS_COUNT_MASK);
            }

            res = whd_proto_set_ioctl(ifp, WLC_SCAN, buffer, 0);
            if (res != WHD_SUCCESS)
            {
                WPRINT_WHD_ERROR( ("%s: legacy scan start failed 0x%lx\n", __func__, (unsigned long)res) );
                whd_driver->internal_info.scan_result_callback = NULL;
                whd_driver->internal_info.whd_scan_result_ptr = NULL;
                whd_wifi_deregister_event_handler(ifp, ifp->event_reg_list[WHD_SCAN_EVENT_ENTRY]);
                ifp->event_reg_list[WHD_SCAN_EVENT_ENTRY] = WHD_EVENT_NOT_REGISTERED;
                return res;
            }

            whd_driver->internal_info.legacy_scan_in_progress = WHD_TRUE;
        }
    }
