...
whd_result_t whd_wifi_on(whd_driver_t whd_driver, whd_interface_t *ifpp)
{
    ...
    *data = 0;
    ...
    {
        /*
         * AP6212 reports tx glom control failure on ART-Pi even when the rest
         * of the firmware bring-up is still healthy. Treat this one knob as
         * optional and let the subsequent control path prove whether the
         * interface is usable.
         */
    ...
    *data = htod32( (uint32_t)1 );
    ...
    else if (retval != WHD_SUCCESS)
    {
        ...
        /*
         * AP6212 on ART-Pi can reject this knob even though STA mode remains
         * usable. Keep the failure visible, but do not abort bring-up here.
         */
    }
    ...
    *ptr = (uint32_t)whd_driver->internal_info.whd_wlan_status.country_code & 0x0000ffff;
    ...
    if (retval != WHD_SUCCESS)
    {
        ...
        /*
         * RT-Thread bring-up succeeds on this module without insisting on the
         * early country iovar. Let later control operations determine whether
         * the interface is actually healthy.
         */
    }
    ...
    * the following IOCTL may fail on initial attempts therefore we try a few times */
    ...
    if (retval != WHD_SUCCESS)
    {
        ...
        /*
         * Some AP6212 firmware builds reject the initial blanket event mask
         * programming but still continue to operate. Keep going so later
         * explicit event registrations can prove the interface.
         */
    }
    ...
    {
        retval = whd_wifi_enable_powersave_with_throughput(ifp, DEFAULT_PM2_SLEEP_RET_TIME);
        if (retval != WHD_SUCCESS)
        {
            WPRINT_WHD_ERROR( ("Failed to configure PM throughput\n") );
        }
    }
