...
static void *whd_msgbuf_get_ioctl_buffer(whd_driver_t whd_driver,
                                         ...
                                         uint16_t data_length)
{
    uint16_t alloc_length = (data_length == 0U) ? (uint16_t)sizeof(uint32_t) : data_length;

    if (whd_host_buffer_get(whd_driver, buffer, WHD_NETWORK_TX, alloc_length,
                            (uint32_t)WHD_IOCTL_PACKET_TIMEOUT) == WHD_SUCCESS)
    {
        uint8_t *data = whd_buffer_get_current_piece_data_pointer(whd_driver, *buffer);
        ...
        if (alloc_length > 0U)
        {
            whd_mem_memset(data, 0, alloc_length);
        }
        return data;
    }
    else
