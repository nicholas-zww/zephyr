...
swap_read_status(struct boot_loader_state *state, struct boot_status *bs)
{
    ...
    BOOT_LOG_ERR("swap: img=%u status_bytes rc=%d", BOOT_CURR_IMG(state), rc);
    if (rc == 0) {
        ...
        BOOT_LOG_ERR("swap: img=%u swap_info=0x%02x swap_type=%d",
                     BOOT_CURR_IMG(state), swap_info, bs->swap_type);
    }
