...
int flash_area_id_from_multi_image_slot(int image_index, int slot)
{
    ...
    switch (slot) {
    case 0:
        id = FLASH_AREA_IMAGE_PRIMARY(image_index);
        BOOT_LOG_INF("slot map image=%d slot=%d -> area=%d", image_index, slot, id);
        return id;
#if !defined(CONFIG_SINGLE_APPLICATION_SLOT)
    case 1:
        id = FLASH_AREA_IMAGE_SECONDARY(image_index);
        BOOT_LOG_INF("slot map image=%d slot=%d -> area=%d", image_index, slot, id);
        return id;
#endif
    }

    BOOT_LOG_ERR("slot map invalid image=%d slot=%d", image_index, slot);
